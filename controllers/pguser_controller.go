...
func (r *PgUserReconciler) updateDatabaseOwnershipAndPrivileges(ctx context.Context, pgApi PgRoleAPI, user *apiV1.PgUser) error {
	...
	for _, database := range user.Spec.Databases {
		...
		if currentOwner != user.Name && database.Owner != nil && *database.Owner { // Case 3: Login Role should be owner of database and is currently not owner of database
			if err := pgApi.UpdateDatabaseOwner(database.Name, user.Name); err != nil {
				logger.Error(err, "Unable to update database owner")
				return err
			}
		} else if currentOwner == user.Name && (database.Owner == nil || !*database.Owner) { // Case 4: Login Role should not be owner of database and is currently owner of database
		...
		if database.Owner == nil || !*database.Owner {
